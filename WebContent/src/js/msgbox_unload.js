(function () {
  ...
  var GenerateHtml = function (type, title, msg) {
    ...
    if (type == "alert") {
      _html += '<input id="mb_btn_ok_unload" type="button" value="确定" />';
    }
    if (type == "judge") {
        _html += '<input id="mb_jug_ok" type="button" value="确定" />';
      }
    if (type == "confirm") {
      _html += '<input id="mb_btn_ok_unload" type="button" value="是" />';
...
//  });
    ...
    $("#mb_btn_ok_unload,#mb_jug_ok,#mb_btn_no").css({ width: '60px', height: '28px', color: '#000', border: '1px solid #fff',borderRadius:'3px',background:'#00aeef',fontSize:'14px', fontFamily:'microsoft yahei' });
    $("#mb_btn_ok_unload,#mb_jug_ok").css({ backgroundColor: '#00aeef' });
