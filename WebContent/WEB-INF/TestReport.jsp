...
<link rel="stylesheet" type="text/css" href="css/testReport.css">
</head>
<body>
	<!-- 	头部开始 -->
		...
		<%@include file="nav.jsp"%>
			...
			<table border="1" cellspacing="0" cellspadding="0" id="table1">
				<tr>
				 ...
				 <c:forEach var="orderInfo" items="${testReport}" varStatus="status">
	 	 ...
	 	 <c:choose>
				<c:when test="${queryType == 'common'}">
					<c:set var="queryUrl"
					value="TestReport?type1=${classify1 }&searchContent1=${parameter1}&selected=${str}&currentPage=">
					</c:set>
				</c:when>
				<c:otherwise>
					<c:set var="queryUrl"
					value="GetTestReportRoute?type1=${classify1 }&searchContent1=${parameter1}&type2=${classify2}&searchContent2=${parameter2}&selected=${str}&currentPage=">
					</c:set>
				</c:otherwise>
	...
	<div class="contract_add">
		<div class="contract_title">添加测试报告信息</div>
		<div class="contractAdd_close">关闭</div>
		<div class="basic_info">
			<div class="table_title">测试报告信息</div>
	...
	<div class="contract_update">
		<div class="contract_title">修改测试报告信息</div>
    ...
    <!--测试报告文档-->
    <div id="test_Report">
    <p class="f16 b m60b">TEST REPORT</p>
    ...
    <div class="content_text m30b">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In accordance with the following captioned letter of credit (DC No.:
        <span contenteditable="true" class="DCNO"> </span>),contract (Contract No.:<span contenteditable="true" class="ContractNO"> SACS-TD-1610-0250/REV.1 </span> )
        <span style="text-decoration:none;" class="PONO_text">and purchase order (PO No.: <span contenteditable="true" class="PONO">E20161121AS001</span> )</span> 
         requirements, we hereby certificate that the material or goods performed by Cascade Microtech is in accordance with all specifications,
          drawings and all other purchase order requirements.
    </div>
    <div class="content_text m30b">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Calibration reports indicating conformance are on file with Cascade Microtech or our suppliers for your examination.</div>
    <div class="content_text m30b">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Cascade Microtech acceptance measurements are traceable to National Standards, to the extent allowed by The National Standards, 
    to the extent allowed by The National Institute of Standards & Technology (N.I.S.T)</div>
    <div class="content_text m30b">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This Equipment has been calibrated in accordance with the test procedures or published specification parameters that apply.
     We further certificate that our quality management system is registered with ISO 9001:2008.</div>
     
    <div style="margin-bottom: 10px;display: inline-block;" class="cf lf">Signature:</div>
	    ...
	    /* 获取隐藏信息 */
	     ...
	     /* else{
	        ...
	        $(".news .PONO").text(PONO);
	     	...
	     	$(".news .PONO_text").html('and purchase order (PO No.:'+$('<span contenteditable="true" class="PONO"></span>')+')' )
	     } */
    $('.hidePdf').show(); 
 })
